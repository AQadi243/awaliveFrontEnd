...
import ImageField from "../AddRooms/ImageField";
import TagSelector from "../AddRooms/TagSelector";
import { useParams } from "react-router-dom";
import { jwtDecode } from "jwt-decode";
import ImageFieldUrl from "./ImageFieldUrl";
...
const EditRoom = () => {
      ...
      if (roomId) {
        try {
          ...
          setFormData({
            ...
            titleAr: room.title.ar,
            subTitleRoomOneEn: room.subTitle.roomOne.en,
            subTitleRoomOneAr: room.subTitle.roomOne.ar,
            maxGuests: room.maxGuests,
            roomQTY: room.roomQTY,
            size: room.size,
            descriptionEn: room.description.en,
            descriptionAR: room.description.ar,
            price: room.priceOptions[0].price,
            priceHistory: room.priceHistory || null,
            tags: room.tags,
            images: room.images
          });
        } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (decodedToken.role !== 'admin') {
      message.error('You are not authorized to update this room.');
      return;
    }
  ...
  return (
    <section>
      <form className="space-y-6 p-4 rounded shadow-lg max-w-3xl mx-auto my-8 border-stroke bg-white dark:border-strokedark dark:bg-boxdark" onSubmit={handleSubmit}>
        <h2 className="text-2xl font-semibold text-center mb-4">Update Room Details</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {/* Images */}
          <ImageFieldUrl images={formData.images} handleImagesChange={handleImagesChange} />
          ...
          <div className="flex justify-center items-center gap-7">
            ...
            {/* Tags */}
            <div className="">
              <label className="block text-sm font-medium text-gray-700 dark:text-white pb-1">
