    ...
    <section className="w-[90%] mx-auto">
        ...
        {loading ? <ImagePlaceholder /> : (
          ...
          <Link data-aos="fade-up" key={room.id} to={`/room/${room.id}`} className="grid-cols-1 cursor-pointer" style={{ position: 'relative' }}>
            ...
            <p className="bg-[#2E2E2E] py-2 px-6 absolute top-5 right-0 text-white text-xs tracking-widest">{t('from')} {currentLanguage === "en"? room.priceOptions[0].price.toLocaleString():room.priceOptions[0].price.toLocaleString('ar-EG')} {t("SAR")}</p>
            <div className="absolute bottom-5 left-2">
              ...
              <div className="flex gap-2 md:gap-3 items-center text-white text-md md:text-xl">
                ...
                <p className='text-sm tracking-widest' >{room.maxGuests} {t('guest')}</p>
