    ...
    <>
      <div className=" flex flex-col-reverse lg:flex-row gap-5 py-5">
        <div className="lg:w-2/3 ">
          ...
          {/* Facilities end  */}
          <hr />
          <p dir={`${currentLanguage === 'ar' ? 'rtl' : 'ltr'}`} className={`py-3 text-sm leading-7 ${currentLanguage === 'ar' ? 'body-ar' : 'body-en'} `}>{description}</p>
          <hr />
          <div dir={`${currentLanguage === 'ar' ? 'rtl' : 'ltr'}`} className=" mb-4">
            <h2 className={`text-2xl  py-5 md:py-4 ${currentLanguage === 'ar' ? 'body-ar' : 'body-en'} `}  >
              {t("roomFeatures")}
            </h2>
